...
class ParkingSpot {
  ...
  static async findAvailableForUserInParking(user_id, parking_id) {
    const query = `
      SELECT DISTINCT ps.*
      FROM parking_spots ps
      JOIN parking_groups pg ON ps.parking_group_id = pg.parking_group_id
      JOIN user_group_parking_group_access ugpga ON pg.parking_group_id = ugpga.parking_group_id
      JOIN user_group_members ugm ON ugpga.group_id = ugm.group_id
      WHERE ugm.user_id = $1 AND pg.parking_id = $2 AND ps.is_occupied = false
    `;
    const result = await pool.query(query, [user_id, parking_id]);
    return result.rows;
  }
}
