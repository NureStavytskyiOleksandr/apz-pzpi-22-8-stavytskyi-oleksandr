...
router.get('/:id', restrictTo('super_admin', 'parking_admin'), async (req, res) => {
  const { id } = req.params;
  try {
...
router.get('/parking/:id', restrictTo('super_admin', 'parking_admin'), async (req, res) => {
  const { id } = req.params;
  try {
    const groups = await ParkingGroup.findByParkingId(id);
    res.json(groups);
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch parking groups by parking_id' });
  }
});
